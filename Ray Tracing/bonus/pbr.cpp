...
// Direct reference to the floor object (no casting needed)
...
bool loadTexture(const char* filename) {
    ...
    if (textureData) {
        ...
        cout << "Dimensions: " << textureWidth << "x" << textureHeight << ", Channels: " << textureChannels << endl;
        return true;
    } else {
...
void loadData()
{
    ...
    for (int i = 0; i < numObjects; i++)
    {
        ...
        if (type == "sphere")
        {
            ...
            // Map traditional parameters to PBR
            double metallic = coefficients[3];  // Use reflection coefficient for metallic
            double roughness = 1.0 - sqrt(coefficients[2]);  // Inverse relationship with specular
            double ao = coefficients[0] * 2.0;  // Ambient occlusion from ambient coefficient
            
            // Create PBR material
            PBRMaterial material(
                Color(color[0], color[1], color[2]),  // Albedo from color
                metallic,
                roughness,
                ao
            );
            temp->setPBRMaterial(material);
        ...
        else if (type == "triangle")
        {
            ...
            // Set PBR material properties
            double metallic = coefficients[3];
            double roughness = 1.0 - sqrt(coefficients[2]);
            double ao = coefficients[0] * 2.0;
    ...
    // Create the floor directly as a Floor object
    floorObject = new Floor(500, 20);
    double floorCoEfficients[] = {0.4, 0.2, 0.2, 0.2};
    double floorShine = 5;
    floorObject->setShine(floorShine);
    floorObject->setCoEfficients(floorCoEfficients);

    // Set PBR material for floor
    PBRMaterial floorMaterial(
        Color(0.8, 0.8, 0.8),  // Light gray albedo
        0.0,                   // Non-metallic
        0.8,                   // Fairly rough
        1.0                    // Full ambient occlusion
    );
    floorObject->setPBRMaterial(floorMaterial);

    // Load the texture but don't enable it initially
    if (loadTexture("sample3.jpg")) {
        floorObject->enableTexture(false);
        cout << "Texture loaded but initially disabled. Press 't' to toggle." << endl;
    }
    ...
    for (int i = 0; i < numPointLights; i++)
    {
        ...
        fin >> x >> y >> z;
        fin >> color[0] >> color[1] >> color[2];
        // Add intensity parameter for PBR (default 5.0 for stronger lighting)
        PointLight pointLight(Vector3D(x, y, z), color, 5.0);
    ...
    for (int i = 0; i < numSpotLights; i++)
    {
        ...
        fin >> color[0] >> color[1] >> color[2];
        fin >> dx >> dy >> dz >> angle;
        // Add intensity parameter for PBR
        SpotLight spotLight(Vector3D(x, y, z), Vector3D(dx, dy, dz), angle, color, 5.0);
...
void keyboardHandler(unsigned char key, int x, int y)
{
    ...
    case 'T':
        if (floorObject) {
            floorObject->useTexture = !floorObject->useTexture;
            cout << "Texture " << (floorObject->useTexture ? "enabled" : "disabled") << endl;
        }
        break;
    case '+':
        // Increase texture scale (less repetition)
        if (floorObject && floorObject->useTexture) {
            floorObject->textureScale *= 1.5;
            cout << "Texture scale increased to: " << floorObject->textureScale << endl;
        }
        break;
    case '-':
        // Decrease texture scale (more repetition)
        if (floorObject && floorObject->useTexture) {
            floorObject->textureScale /= 1.5;
            if (floorObject->textureScale < 0.1) floorObject->textureScale = 0.1;
            cout << "Texture scale decreased to: " << floorObject->textureScale << endl;
        }
        break;
    // PBR parameter controls
    case 'm':
        // Increase metallic for selected object
        if (!objects.empty() && objects[0]) {
            double metallic = objects[0]->pbrMaterial.metallic + 0.1;
            if (metallic > 1.0) metallic = 1.0;
            objects[0]->setMetallic(metallic);
            cout << "Metallic increased to: " << objects[0]->pbrMaterial.metallic << endl;
        }
        break;
    case 'n':
        // Decrease metallic for selected object
        if (!objects.empty() && objects[0]) {
            double metallic = objects[0]->pbrMaterial.metallic - 0.1;
            if (metallic < 0.0) metallic = 0.0;
            objects[0]->setMetallic(metallic);
            cout << "Metallic decreased to: " << objects[0]->pbrMaterial.metallic << endl;
        }
        break;
    case 'r':
        // Increase roughness for selected object
        if (!objects.empty() && objects[0]) {
            double roughness = objects[0]->pbrMaterial.roughness + 0.1;
            if (roughness > 1.0) roughness = 1.0;
            objects[0]->setRoughness(roughness);
            cout << "Roughness increased to: " << objects[0]->pbrMaterial.roughness << endl;
        }
        break;
    case 'f':
        // Decrease roughness for selected object
        if (!objects.empty() && objects[0]) {
            double roughness = objects[0]->pbrMaterial.roughness - 0.1;
            if (roughness < 0.0) roughness = 0.0;
            objects[0]->setRoughness(roughness);
            cout << "Roughness decreased to: " << objects[0]->pbrMaterial.roughness << endl;
        }
        break;
    case 27:
        for (Object *object : objects)
...
int main(int argc, char **argv)
{
    ...
    // Clean up texture data before exiting
    if (textureData) {
        stbi_image_free(textureData);
        textureData = nullptr;
    }
