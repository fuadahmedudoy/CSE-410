...
GLfloat eyex = 15, eyey = 15, eyez = 15;          // Camera position coordinates
GLfloat centerx = 5, centery = 5, centerz = 5; // Look-at point coordinates
...
GLfloat rotationAxisx=0,rotationAxisy=0,rotationAxisz=0,rotationAngle=0; // Sphere rotation axis
...
void timerFunction(int value) {
    if(isMovingSphere) {
        sphereMechanics(); // Update sphere position and velocity
    }
    // if(showVector){
    //     printf("showVector\n");
    //     drawVelocityVector(); // Draw velocity vector
    // }
    glutPostRedisplay(); // Request a screen refresh
    glutTimerFunc(16, timerFunction, 0); // Call this function again after 16 ms (approx. 60 FPS)
}
...
int main(int argc, char **argv)
{
    ...
    glutTimerFunc(0, timerFunction, 0); // Start the timer for animation
